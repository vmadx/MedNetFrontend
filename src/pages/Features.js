...
import { motion } from 'framer-motion';
import { 
  Zap, Star, Play, Sparkles, 
  Database, Target, ChevronRight, Eye,
  Cpu, Layers, Activity, GitBranch
} from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        >
          ...
          <motion.p 
            ...
            variants={itemVariants}
          >
            Discover the multi-scale capabilities that make MED-Net a state-of-the-art solution for ECG anomaly detection
          </motion.p>
        </motion.div>
        ...
        >
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div>
              <h3 className="text-xl font-semibold text-slate-900 dark:text-slate-100 mb-4 flex items-center space-x-2">
                <Database className="w-5 h-5 text-teal-600 dark:text-teal-400" />
                <span>Architecture Enhancements</span>
              </h3>
              <ul className="space-y-2 text-slate-800 dark:text-slate-300">
                <li className="flex items-center space-x-2">
                  <div className="w-2 h-2 bg-teal-600 rounded-full"></div>
                  <span>Enhanced Window Size (128 vs 10) </span>
                </li>
                <li className="flex items-center space-x-2">
                  <div className="w-2 h-2 bg-teal-600 rounded-full"></div>
                  <span>Learning Rate Decay Optimization </span>
                </li>
                <li className="flex items-center space-x-2">
                  <div className="w-2 h-2 bg-teal-600 rounded-full"></div>
                  <span>CPU-Optimized (Run on Intel i5) </span>
                </li>
                <li className="flex items-center space-x-2">
                  <div className="w-2 h-2 bg-teal-600 rounded-full"></div>
                  <span>Enhanced Attention Refinement Layer </span>
                </li>
              </ul>
            </div>
            ...
            <div>
              <h3 className="text-xl font-semibold text-slate-900 dark:text-slate-100 mb-4 flex items-center space-x-2">
                <Target className="w-5 h-5 text-teal-600 dark:text-teal-400" />
                <span>Performance (ECG Data)</span>
              </h3>
              <ul className="space-y-2 text-slate-800 dark:text-slate-300">
                <li className="flex items-center space-x-2">
                  <div className="w-2 h-2 bg-green-600 rounded-full"></div>
                  <span className="font-semibold">99.97% F1-Score</span>
                  <span className="text-sm text-slate-500 ml-2">(vs 99.4% Baseline) </span>
                </li>
                <li className="flex items-center space-x-2">
                  <div className="w-2 h-2 bg-green-600 rounded-full"></div>
                  <span>100% Recall Rate </span>
                </li>
                <li className="flex items-center space-x-2">
                  <div className="w-2 h-2 bg-green-600 rounded-full"></div>
                  <span>0.9996 ROC-AUC </span>
                </li>
                <li className="flex items-center space-x-2">
                  <div className="w-2 h-2 bg-green-600 rounded-full"></div>
                  <span>50% Reduced Training Time </span>
                </li>
              </ul>
            </div>
          </div>
        ...
        >
          ...
          <h2 className="text-3xl font-bold mb-6">Experience MED-Net in Action</h2>
          <p className="text-xl leading-relaxed mb-8 max-w-3xl mx-auto">
            See how our multi-scale enhanced model detects cardiac anomalies in real-time with our interactive demonstration.
          </p>
          <motion.a
            whileHover={{ scale: 1.05, y: -2 }}
            whileTap={{ scale: 0.95 }}
